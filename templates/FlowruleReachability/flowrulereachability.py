...
class Mechanism:        # Class for defining Mechanism Should be named Mechanism, must follow naming conventions
    ...
    def getFlowRuleHopCounts(self, kg_relations):
        ...
        for h1 in self.hosts.getHosts()["hosts"]:
            for h2 in self.hosts.getHosts()["hosts"]:
                q1 = "Match (h1:Host{mac:\'" + h1.get('mac',
                                                      '') + "\'})-[r1:" + host_switch_rel + "]-(s1:Switch) return s1,properties(r1)"
                data1 = DBUtil.execute_query(q1)
                # source mac not found
                if (len(data1) == 0):
                    print(colored("Source Mac-" + h1.get('mac', '') + " not found in KG", 'red'))
                ...
                q1 = "Match (h2:Host{mac:\'" + h2.get('mac',
                                                      '') + "\'})-[r2:" + host_switch_rel + "]-(s2:Switch) return s2,properties(r2)"
                data1 = DBUtil.execute_query(q1)
                ...
                while (True):
                    # find whether there is a flow rule matching source id and destination id in the current switch's flow table
                    q2 = "MATCH (s1:Switch{id:" + "\'" + switchId + "\'" + "})-[hc:" + switch_flowtable_rel + "*]->(f:FlowTable)-[:" + flowtable_flow_rel + "]->(f1:Flow)-[:" + flow_match_rel + "]->(m1:Match)-[:" + match_eth_rel + "]->(e1:EthAddress) where e1.dst=" + "\'" + h2.get(
                        'mac', '') + "\'" + " and e1.src=" + "\'" + h1.get('mac', '') + "\'" + " return f1,m1,e1"
                    data2 = DBUtil.execute_query(q2)
                    # if there is no flow rule in the flow table in the current switch under consideration, return false
                    if (len(data2) == 0):
                        print(colored("No matching flow rule in Switch" + switchId, 'red'))
                    ...
                    # if the ingress port information is missing, return False
                    if (len(data3) == 0):
                        print(colored("For Source:" + h1.get('mac', '') + "and Desrination" + h2.get('mac',
                                                                                                     '') + " No matching flow rule in Switch" + switchId,
                                      'red'))
                        break
                    ...
                    if (len(data4) == 0):
                        print(colored("For Source:" + h1.get('mac', '') + "and Desrination" + h2.get('mac',
                                                                                                     '') + " No forwarding instruction available in Switch" + switchId,
                                      'red'))
                        break
                    ...
                    if (not (out_type == 'OUTPUT')):
                        print(colored("For Source:" + h1.get('mac', '') + "and Desrination" + h2.get('mac',
                                                                                                     '') + " Flow rule instruction in Switch" + switchId + " blocks the packet",
                                      'red'))
                        break
                    ...
                    # 2. the output port matches the port to which destination host is connected
                    if (switchId == destId and out_port == destPort):
                        print(colored(h1.get('mac', '') + " reaches " + h2.get('mac', ''),'green'),"\t",colored("with Hop Count: "+str(hopCount),'blue'))
                        break
                    ...
                    if (len(data5) == 0):
                        ...
                        if (len(data5) == 0):
                            print(colored("For Source:" + h1.get('mac', '') + "and Desrination" + h2.get('mac',
                                                                                                         '') + "No link origination from Switch" + switchId + " from port " + out_port,
                                          'red'))
                            break
